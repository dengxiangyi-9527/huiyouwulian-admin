  ...
  <div class="app-container">
      ...
      <el-dialog :visible.sync="specVisiable" title="设置规格">
        <el-form ref="specForm" :rules="rules" :model="specForm" status-icon label-position="left"
          ...
          <el-form-item label="规格图片" prop="picUrl">
            <el-upload :action="uploadPath" :show-file-list="false" :on-success="uploadSpecPicUrl"
              class="avatar-uploader" accept=".jpg,.jpeg,.png,.gif">
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="handleSpecificationAdd" v-db-click>确定</el-button>
        </div>
      </el-dialog>
    </el-card>
    ...
    <el-card class="box-card">
      ...
      <el-table :data="products">
        ...
        <el-table-column property="productPowerType" width="100" label="产品类型" />
        <el-table-column property="shareProfitRatio" width="100" label="分享收益%" />
        ...
        <el-table-column align="center" label="操作" width="100"
          class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="handleProductShow(scope.row)">设置
            </el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-dialog :visible.sync="productVisiable" title="设置货品">
        <el-form ref="productForm" :model="productForm" status-icon label-position="left"
          label-width="100px" style="width: 400px; margin-left:50px;">
          ...
          <el-form-item label="计算值" prop="calnumber">
            <el-input v-model="productForm.calnumber" />
          </el-form-item>
          <el-form-item label="产品类型">
            <el-select v-model="productForm.productPowerType">
              <el-option v-for="item in productPower" :key="item.id" :label="item.name"
                :value="item.id" />
            </el-select>
          </el-form-item>
          <el-form-item label="分享收益%" prop="calnumber">
            <el-input v-model="productForm.shareProfitRatio"
              @blur="watchShareProfitRatio(productForm.shareProfitRatio)"
              placeholder="请输入11-29之间" />
          </el-form-item>
          <el-form-item label="货品图片" prop="url">
            <el-upload :action="uploadPath" :show-file-list="false" :on-success="uploadProductUrl"
              class="avatar-uploader" accept=".jpg,.jpeg,.png,.gif">
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="handleProductEdit" v-db-click>确定</el-button>
        </div>
      </el-dialog>
    </el-card>
...
<script>
import { publishGoods, listCatAndBrand } from '@/api/mall/goods'
  ...
  methods: {
    ...
    watchShareProfitRatio (val) {
      ...
      if (val < 11 || val > 29) {
        ...
        this.productForm.shareProfitRatio = ''
      }
      this.productForm.shareProfitRatio = val
    },
    handleClose (tag) {
