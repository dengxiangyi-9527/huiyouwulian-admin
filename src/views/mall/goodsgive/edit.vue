  ...
  <div class="app-container">
    <el-card class="box-card">
      ...
      <el-table :data="addGoodsList" style="width: 100%;">
        <el-table-column property="name" width="400" label="名称" />
        <el-table-column property="url" width="400" label="图片">
...
<script>
  import { getObj, putObj } from  '@/api/mall/mallgoodsgive'
  ...
  export default {
    ...
    methods: {
      init() {
        ...
        getObj(goodsId).then(res => {
          this.goods = res.data.mallGoodsGive
          this.addGoodsList.push({ name: this.goods.goodsName, url: this.goods.goodsPicUrl })
          this.addGiveGoodsList.push({ name: this.goods.giveGoodsName, url: this.goods.giveGoodsPicUrl })
          this.time = this.goods.validEndTime
          this.number = this.goods.giveGoodsNumber
        })
      },
      selGoods () {
      ...
      handlePublish: function () {
        ...
        putObj(finalGoods).then(response => {
